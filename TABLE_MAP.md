...
| Table 9 | Large-pool stress test — **1,323** de-duplicated pool; baselines on the **636**-case CT sub-pool | same three scripts | `results/retrieval/dedup/retrieval_tab8_stress.json` + `baselines_tab7_tab8.json` (`tab8_subpool`); stratum-(c) per-metric CIs: `tab9c_metric_cis.json` | A1 / C |
...
**dedup file is canonical** (de-duplicated 1,323 pool after the 09-20 census + 09-22 CT-voxel second pass — `results/audit/axis_normalized_twin_census.json` incl. §second_pass; the paper's numbers). Frozen originals
