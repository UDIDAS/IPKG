...
KG = os.environ.get("VKG_KG", os.path.join(_ROOT, "kg_graphs"))      # bundle-relative (originally /home/user/SWOG/kg)
DATA = os.environ.get("VKG_CORPORA", os.path.join(_ROOT, "corpora"))
OUT = KG
SCHEMA = os.path.join(KG, "schema.owl") if os.path.exists(os.path.join(KG, "schema.owl")) else os.path.join(KG, "schema.ttl")
...
ORGAN_MAPKEY = {"liver": "Organ::Liver", "pancreas": "Organ::Pancreas", "spleen": "Organ::Spleen",
                ...
                "kidney": "Organ::Kidney"}                      # side-agnostic kidney (KiTS, pooled FLARE23)
...
TUMOR_MAPKEY = {"pancreas": "Lesion::Pancreatic tumor", "liver": "Lesion::Liver tumor",
                "kidney": "Lesion::Renal tumor", "right_kidney": "Lesion::Renal tumor", "left_kidney": "Lesion::Renal tumor"}
TUMOR_CLASS = {"pancreas": "PancreaticTumor", "liver": "LiverTumor",
               "kidney": "RenalTumor", "right_kidney": "RenalTumor", "left_kidney": "RenalTumor"}
...
SUPP = {
    ...
    "Organ::Kidney": {"SNOMED CT": ("64033007", "Kidney structure"), "NCIt": ("C12415", "Kidney")},
    # renal tumor (added 2026-09-14; verified by lookup — SNOMED CT US 2026-03-01 via NCI EVS, NCIt current):
    # neoplasm-level concepts, parallel to Lesion::Pancreatic tumor (C3305 Pancreatic Neoplasm).
    # NCIt C9385 "Renal Cell Carcinoma" is histology-specific and was not used.
    "Lesion::Renal tumor": {"SNOMED CT": ("126880001", "Neoplasm of kidney"), "NCIt": ("C3150", "Kidney Neoplasm")},
}
...
def main():
    ...
    g = Graph()
    g.parse(SCHEMA)                                   # KG includes the ontology
    ...
    for mk, systems in SUPP.items():                   # fallback only: the JSON is the single source (2026-09-15)
