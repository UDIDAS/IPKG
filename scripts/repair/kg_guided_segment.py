...
ATLAS = os.path.join(VKG_DATA, "kg_atlas.json")   # originally /home/user/SWOG/kg/data/kg_atlas.json
...
def load_atlas(path=ATLAS):
    """Organ volume bands (p1..p99) used only to FLAG implausible volumes; the mask is never edited by them.
    Missing atlas -> no volume flags (Dice results are unaffected)."""
...
def repair(mask, spacing, atlas=None, verbose=False):
    ...
    atlas = load_atlas() if atlas is None else atlas
...
def _self_check():
    ...
    cases = sorted(glob.glob(os.path.join(VKG_DATA, "flare_full_cases", "*_label.nii.gz")))
    if not cases:
