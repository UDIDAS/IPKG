...
# Bundle-local variant (2026-09-13): corpora from ../../corpora via retrieval_core_local, model id + output
# tag from argv, device_map=auto (multi-GPU), Qwen3 thinking disabled. Protocol, prompt, sampling and
# metrics are unchanged from llm_expert.py.  Usage: python llm_expert_local.py <hf-model-id> [<tag>]
...
from retrieval_core_local import similarity, relevant, load_corpus
core.ORGAN_UNIVERSE = ["liver", "spleen", "pancreas", "kidney"]
RES = os.path.join(_HERE, "..", "..", "results", "llm_expert")
rng = np.random.default_rng(0)

def load(kind):
    return load_corpus(kind, datasets=["kits", "lits", "msd"])
...
out["pair_labels"] = {f"{q}|{b}": v for (q, b), v in lab.items()}
json.dump(out, open(f"{RES}/llm_expert_study_{TAG}.json", "w"), indent=2)
print("\n=== LLM-as-expert study ===")
